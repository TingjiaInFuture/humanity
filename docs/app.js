(() => {
  ...
  const I18N = {
    "zh-CN": {
      ...
      logEyebrow:"观察记录", logTitle:"这份记录会一直更新下去。", logCopy:"下面是维护者定期记录的观察，每条都注明来源。日期是记录日，不是事件日；历史条目不回改——发现错了，就新起一条更正。", logDiscussCta:"对某一条有异议或补充？带着来源来讨论 ↓", logEmpty:"暂无记录。", logCorrected:"已更正",
      ...
      submitting:"正在提交…", discussionSuccess:"讨论已创建：", changeSuccess:"提案已提交：", issueOnly:"提案收到了，但需要人工处理：", issueOnlyReason:"你选的文字在最新版本里找不到（可能已经被别人改过）。可以点开提案补充说明，或者回来重新选一次。", invalidForm:"还有必填项没填完，记得勾选同意公开。", captchaRequired:"请先完成人机验证。", apiNotConfigured:"前端尚未配置 Worker 地址。请编辑 docs/config.js。", requestFailed:"提交失败了，请稍后再试。", activityFailed:"暂时读不到公开记录。", discussionKind:"讨论", changeKind:"修改", discussionLink:"查看讨论", issueLink:"查看提案", prLink:"查看修改详情"
    },
    en: {
      ...
      logEyebrow:"Observation log", logTitle:"This log keeps updating.", logCopy:"Observations recorded by the maintainer from time to time, each with sources. Dates are recording days, not event days; past entries are never rewritten — a mistake gets a fresh correction entry.", logDiscussCta:"Disagree with an entry, or have something to add? Bring sources ↓", logEmpty:"No entries yet.", logCorrected:"Corrected",
  ...
  function renderLog(){
    ...
    root.innerHTML=entries.map(e=>{
      ...
      const badge=corrected?`<span class="log-corrected-badge">${esc(t("logCorrected"))}</span>`:"";
      const srcs=(e.sources||[]).map(s=>`<a href="${esc(s.url)}" target="_blank" rel="noreferrer">${esc(s.label)}</a>`).join("");
      return `<article class="log-item${corrected?" corrected":""}"><span class="log-date">${esc(e.date||"")}</span><div><h3 class="log-title">${badge}${esc(title)}</h3><p class="log-body">${esc(body)}</p>${srcs?`<p class="log-sources">${srcs}</p>`:""}</div></article>`;
    }).join("");
  }
